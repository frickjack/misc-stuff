...
testSecretCreate() {
    ...
    local secretId
    if secretInfo="$(little secret lookup $secretName)"; then
    ...
    secretInfo="$(little secret create $secretName "$value" 'test secret')"; because $? "create secret worked $secretInfo"
    local secretId
